...
class PokerNetwork(nn.Module):
    ...
    def __init__(self, input_size=123, hidden_size=256, num_actions=5, num_discards = NUM_DISCARD_CLASSES):
        super().__init__()
        ...
        self.discard_head = nn.Linear(hidden_size, num_discards)
    ...
    def forward(self, x, opponent_features=None):
        ...
        discard_logits = self.discard_head(features)
        
        # Output bet size between 0.0 and 3.0 (representing pot multiplier)
        bet_size = 0.0 + 3.0 * self.sizing_head(features)
        
        return action_logits, bet_size, discard_logits
...
def encode_state(obs_dict):
    ...
    position_vec = np.array([1.0 if blind_pos == 1 else -1.0], dtype=np.float32)
